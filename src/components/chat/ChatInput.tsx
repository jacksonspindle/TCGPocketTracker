...
export default function ChatInput({ cards, sets }: ChatInputProps) {
  ...
  const collectionSummary = useCollectionSummary(cards, collection);
    ...
    <form onSubmit={handleSubmit} className="p-4 border-t border-gray-200 bg-white">
      <div className="flex gap-2">
        <input
          ...
          disabled={isLoading}
          className="flex-1 px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-400 disabled:bg-gray-50 disabled:text-gray-400"
        />
        <button
          type="submit"
          disabled={!input.trim() || isLoading}
          className="px-4 py-3 bg-teal-500 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed hover:bg-teal-600 transition-colors"
        >
